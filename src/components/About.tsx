...
import { Award, Building, Target, TrendingUp, Users, Zap } from "lucide-react";
...
const About = () => {
  const backgroundRef = useParallaxBackground(1.25);
  const overlayRef = useParallaxContent('up', 0.7);
  const floatingLeftRef = useParallaxHorizontal('left', 1.1);
  const floatingRightRef = useParallaxHorizontal('right', 1.1);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, hsl(var(--primary)) 0%, transparent 40%, hsl(var(--accent)) 60%, transparent 100%)",
        }}
      />

      {/* Floating decorative elements with parallax */}
      <div ref={floatingRightRef as any} className="absolute top-16 right-16 opacity-30">
        <Building className="w-12 h-12 text-accent" />
      </div>
      <div ref={floatingLeftRef as any} className="absolute bottom-16 left-16 opacity-35">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div ref={contentRef as any} className="scroll-reveal-left">
            ...
            <p className="text-lg text-muted-foreground mb-6 leading-relaxed hover:text-foreground transition-colors duration-300">
              Founded on the principles of excellence and innovation, Aurex &
              Fields has become the premier destination for luxury real estate
              marketing. Our team combines decades of market expertise with
              cutting-edge marketing strategies to deliver unparalleled results.
            </p>

            <p className="text-lg text-muted-foreground mb-8 leading-relaxed hover:text-foreground transition-colors duration-300">
              We understand that luxury real estate is more than just
              property—it's about lifestyle, aspiration, and creating lasting
              impressions. Every campaign we craft tells a story that resonates
              with discerning buyers.
            </p>

            <Button
              size="lg"
              className="btn-luxury hover:scale-105 transition-transform animate-pulse-slow"
            >
              Learn Our Story
            </Button>
          </div>
          ...
          <div
            ref={statsRef as any}
            className="grid grid-cols-2 gap-6 scroll-reveal-right"
          >
            {stats.map((stat, index) => (
              <Card
                key={index}
                className={`glass hover:shadow-gold transition-all duration-500 hover:-translate-y-2 hover:rotate-2 border-border/20`}
              >
